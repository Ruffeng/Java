...
public class PR2_Ex3_1_Test {
    ...
    public void createSubmission() throws InterruptedException {        
       Organizer o1 = u1.asOrganizer();
        ...
        assertNotNull(s1);
        assertEquals(comp1_pred1, s1.getPrediction(), 0.001);
        assertEquals(SubmissionStatus.PENDING, s1.getStatus());        
        
        Thread.sleep(1000);  
        Submission s2 = p1.submitPrediction(c1, comp1_pred1);        
        assertThat(s1, is(not(equalTo(s2))));
        
        Competition c2 = o1.newCompetition(comp_title2, comp_target2);        
        Submission s3 = p1.submitPrediction(c2, comp2_pred1);
       
        assertEquals(2, c1.getSubmissions().size());
        assertEquals(1, c2.getSubmissions().size());
        assertEquals(3, p1.getSubmissions().size()); 
